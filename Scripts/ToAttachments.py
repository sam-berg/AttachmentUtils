    ...
    try:
        ...
        with open(matchtable, 'wb') as f:
            ...
            if type == "blob":
                ...
                with arcpy.da.SearchCursor(in_dataset, ["OID@", field,'ATT_NAME']) as scur:
                    # Read through the dataset, harvest files from blob, write
                    # to folder, then keep track of OIDs
                    for row in scur:
                        try:
                            #SBTEST path = os.path.join(str(filedir),
                            #"file_{0}.{1}".format(row[0], ftype))
                            #arcpy.AddMessage("debug")
                            path = os.path.join(str(filedir), "{0}".format(row[2]))	
                    ...
                    # Enable geodatabase attachments and write intermediate
                    # files to gdb
                    f.close()
                    arcpy.AddMessage("Enabling Attachments for: " + in_dataset)
                ...
                with arcpy.da.SearchCursor(in_dataset, ["OID@"]) as scur:
                    ...
                    for row in scur:
                        try:
                            ...
                            #SBTEST newname = os.path.join(str(filedir),
                            #"image_{0}.jpg".format(row[0]))
                            newname = os.path.join(str(filedir), "{0}".format(row[2]))	
            ...
            # If working with a file path or hyperlink
                ...
                with arcpy.da.SearchCursor(in_dataset, ["OID@", field]) as scur:
                    # Need to read the first value of the input field to see if
                    # it is a full path, weblink, or path relative to the
                    # hyperlink base
                    for row in scur:
                        path = row[1]
                        scur.reset()
                        break
                    # If the path 'exists', it is a file on disk or network
                    # location
                    ...
                    # If the path doesn't exist, check if it is on the web or
                    # relative to the hyperlink base
                    else:
                        # On the web
                        if str(path).lower().find("http") > -1 or str(path).lower().find("www") > -1:
                            for row in scur:
                                path = row[1]
                                try:
                                    # Go through search cur, download file and
                                    # write oid and new path
                                    ...
                                    arcpy.SetProgressorPosition()
                            # Enable geodatabase attachments and write
                            # intermediate files to gdb
                            f.close()
                            arcpy.management.EnableAttachments(in_dataset)
                            arcpy.management.AddAttachments(in_dataset, oidfield, matchtable, "OID", "FILE")
                        # Relative to hyperlink base?
                            ...
                            if hyperlinkDir:
                                # If the hyperlinked path exists it is a file
                                # on disk or network location
                                ...
                                # Else, the hyperlink path might be to web
                                    ...
                                    for row in scur:
                                        ...
                                        try:
                                            # Go through search cur, download
                                            # file and write oid and new path
                                            u = urllib2.urlopen(path)
                                            newname = os.path.join(str(filedir), os.path.basename(path))
                                            localFile = open(newname, "wb")
                                            localFile.write(u.read())
